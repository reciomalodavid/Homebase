...
function bindProtectedActions(){
 ...
 if(syncNowBtn&&typeof refreshFromCloud==='function'&&typeof writeCloud==='function'){
  syncNowBtn.onclick=async()=>{try{await ensureAnonymousAuth();await refreshFromCloud(true);await writeCloud()}catch(error){console.error('Production secure sync',error);setStatus(errorText(error),true)}};
 }
}
...
async function start(){
 ...
 try{
  ...
  if(state?.syncCode){
   ...
   setStatus('Autenticación activa · todavía no hay hogar vinculado.');
  }
  ...
  console.error('Homebase production auth',error);setStatus(errorText(error),true);
  window.dispatchEvent(new CustomEvent('homebase:auth-error',{detail:{message:String(error?.message||error)}}));
 }
}

window.HOMEBASE_SECURITY={version:VERSION,getUid:()=>currentUid,getAuthorizedUids:()=>[...authorizedUids],ensureAuth:ensureAnonymousAuth,enroll:enrollCurrentDevice};
