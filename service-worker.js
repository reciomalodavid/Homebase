const SHELL_CACHE="homebase-longpress-v2-ios";
const RUNTIME_CACHE="homebase-longpress-v2-ios";
...
self.addEventListener("activate",e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("homebase-longpress-v2-ios")&&![SHELL_CACHE,RUNTIME_CACHE].includes(k)).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
