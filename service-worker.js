const SHELL_CACHE="homebase-longpress-v1";
const RUNTIME_CACHE="homebase-longpress-v1";
...
self.addEventListener("activate",e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("homebase-longpress-v1")&&![SHELL_CACHE,RUNTIME_CACHE].includes(k)).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
