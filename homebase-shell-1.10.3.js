(() => {
  'use strict';

  let lastY = Math.max(0, window.scrollY || 0);
  let ticking = false;
  let accumulatedDown = 0;
  let accumulatedUp = 0;

  function topbar(){ return document.querySelector('.topbar'); }

  function showHeader(){
    const el = topbar();
    if (el) el.classList.remove('hb-header-hidden');
  }

  function hideHeader(){
    const el = topbar();
    if (el) el.classList.add('hb-header-hidden');
  }

  function update(){
    ticking = false;
    const y = Math.max(0, window.scrollY || document.documentElement.scrollTop || 0);
    const delta = y - lastY;

    if (y <= 28){
      accumulatedDown = 0;
      accumulatedUp = 0;
      showHeader();
      lastY = y;
      return;
    }

    if (delta > 0){
      accumulatedDown += delta;
      accumulatedUp = 0;
      if (y > 90 && accumulatedDown > 18) hideHeader();
    } else if (delta < 0){
      accumulatedUp += Math.abs(delta);
      accumulatedDown = 0;
      if (accumulatedUp > 8) showHeader();
    }

    lastY = y;
  }

  function onScroll(){
    if (!ticking){
      ticking = true;
      requestAnimationFrame(update);
    }
  }

  window.addEventListener('scroll', onScroll, { passive:true });
  window.addEventListener('pageshow', showHeader);
  document.addEventListener('click', event => {
    if (event.target.closest('dialog,.modal')) return;
    if ((window.scrollY || 0) <= 28) showHeader();
  }, true);

  const observer = new MutationObserver(() => {
    const el = topbar();
    if (el && (window.scrollY || 0) <= 28) el.classList.remove('hb-header-hidden');
  });
  observer.observe(document.documentElement,{childList:true,subtree:true});
})();
