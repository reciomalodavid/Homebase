...
function renameMoreToManagement(){
  ...
  if(nav){const icon=nav.querySelector('span');nav.childNodes.forEach(node=>{if(node.nodeType===Node.TEXT_NODE&&node.textContent.trim()==='Más')node.textContent='Gestión'});nav.setAttribute('aria-label','Gestión');if(icon&&text(nav).includes('Más'))nav.innerHTML=icon.outerHTML+'Gestión'}
  const title=document.querySelector('#morePage .hero-row h1,#morePage h1');if(title&&text(title)==='Más')title.textContent='Gestión';
}
...
function allProfiles(){
  const out=[];
  try{const stored=JSON.parse(localStorage.getItem('homebase_profiles')||'[]');if(Array.isArray(stored))out.push(...stored)}catch{}
  if(Array.isArray(window.PEOPLE))out.push(...window.PEOPLE);
  const seen=new Set();
  return out.filter(profile=>{const key=norm(profile?.name||profile?.id||profile?.uuid||'');if(!key||seen.has(key))return false;seen.add(key);return true});
}
function profileAliases(){
  const map=new Map();
  for(const profile of allProfiles()){
    const name=String(profile?.name||'').trim();if(!name)continue;
    [profile.id,profile.uuid,profile.stableId,profile.key,profile.slug,profile.name].filter(Boolean).forEach(value=>map.set(norm(value),name));
  }
  return map;
}
function resolvePersonValue(value,aliases){
  if(value==null)return[];
  if(Array.isArray(value))return value.flatMap(entry=>resolvePersonValue(entry,aliases));
  if(typeof value==='object'){
    const direct=value.name||value.label||value.displayName||value.title;
    const ids=[value.id,value.uuid,value.profileId,value.memberId,value.personId].filter(Boolean);
    return uniqueMeaningful([direct,...ids.map(id=>aliases.get(norm(id))||'')]);
  }
  const raw=String(value).trim();if(!raw)return[];
  return uniqueMeaningful([aliases.get(norm(raw))||raw]);
}
function collectPersonNames(item,aliases){
  const keys=['people','person','personId','personIds','peopleIds','owner','ownerId','owners','assignee','assignees','assigneeId','assigneeIds','member','members','memberId','memberIds','profile','profileId','profileIds'];
  const values=[];for(const key of keys)if(item?.[key]!=null)values.push(item[key]);
  const walk=(obj,depth=0)=>{if(!obj||typeof obj!=='object'||depth>2)return;for(const [key,value] of Object.entries(obj)){if(/person|people|owner|assignee|member|profile/i.test(key))values.push(value);if(value&&typeof value==='object'&&!Array.isArray(value))walk(value,depth+1)}};
  walk(item);
  return uniqueMeaningful(values.flatMap(value=>resolvePersonValue(value,aliases)));
}
function searchablePersonAliases(item,people,aliases){
  const out=[...people];
  const inspect=value=>{if(value==null)return;if(Array.isArray(value)){value.forEach(inspect);return}if(typeof value==='object'){Object.values(value).forEach(inspect);return}const resolved=aliases.get(norm(value));if(resolved)out.push(resolved)};
  inspect(item);
  return uniqueMeaningful(out);
}
function profilesMap(){const map=new Map();for(const profile of allProfiles()){const name=String(profile?.name||'');[profile.id,profile.uuid,profile.stableId,profile.name].filter(Boolean).forEach(key=>map.set(String(key),name))}return map}
function coreSearchRows(){
  ...
  return state.items.filter(item=>!item?.deletedAt).map(item=>{
    ...
    const people=collectPersonNames(item,aliases);
    const personSearch=searchablePersonAliases(item,people,aliases);
    ...
    const haystack=norm([title,item.notes,...personSearch,category,item.categoryOther,item.date,item.endDate,r.code,r.kind,r.route,r.hotel,r.hotelAirport,flights].filter(Boolean).join(' '));
...
function expirySearchRows(){
  ...
  return expiries.map(item=>{const owner=item.profileName||profiles.get(String(item.profileId||''))||'';const title=item.title||'Vencimiento';const meta=uniqueMeaningful([owner,item.notes]).join(' · ');const haystack=norm([title,owner,item.notes,item.expiryDate].filter(Boolean).join(' '));return {type:'Vencimiento',title,date:item.expiryDate||'',meta,haystack,itemId:String(item.id||''),target:'morePage'}});
}
function searchRows(query){
  const q=norm(query);if(q.length<2)return[];const terms=q.split(' ').filter(Boolean);
  return [...coreSearchRows(),...expirySearchRows()].filter(row=>terms.every(term=>row.haystack.includes(term))).sort((a,b)=>{if(a.date&&b.date)return a.date.localeCompare(b.date);if(a.date)return -1;if(b.date)return 1;return a.title.localeCompare(b.title,'es')}).slice(0,SEARCH_LIMIT);
}
...
function renderSearch(){
  ...
  const q=input.value.trim();if(q.length<2){results.innerHTML='<div class="bgs-empty">Escribe al menos 2 letras para buscar.</div>';if(count)count.textContent='';return}
