...
function routeAll(r){
  ...
  const out=[];
  for(const f of fs){
    if(!out.length&&f.dep)out.push(f.dep);
    if(f.arr)out.push(f.arr);
  }
  return out.join('–');
}
...
function compactLines(items){
  ...
  for(const item of items){
    ...
    if(k==='flight'||k==='dh'){
      lines.push({cls:'flight',main:'VUELO',times:ciCo(r,item),route:routeAll(r),flights:flightNumbers(r),night:nightCity(r)});
    }else if(k==='off'||OFF_CODES.test(norm(r.code))){
      lines.push({cls:'off',main:'OFF',times:'',route:'',flights:'',night:''});
    }else if(k==='vacation'||norm(r.code)==='U'){
      lines.push({cls:'vacation',main:'VACACIONES',times:'',route:'',flights:'',night:''});
    }else{
      const place=r.airport||r.ciAirport||r.showUpAirport||'';
      lines.push({cls:kindClass(r),main:simpleDutyLabel(r),times:ciCo(r,item),route:place,flights:'',night:nightCity(r)});
    }
  }
