(()=>{
  ...
  function enhanceReminderForm(form){
    ...
    const applyCustom=()=>{
      ...
      if(!option){option=new Option('',String(days));option.dataset.customReminder='1';select.add(option)}
      option.value=String(days);option.textContent=`${days} días antes`;select.value=String(days);wrap.classList.remove('open');return true;
    };
    select.addEventListener('change',()=>{const custom=select.value==='__custom';wrap.classList.toggle('open',custom);if(custom)requestAnimationFrame(()=>input.focus())});
    input.addEventListener('change',applyCustom);
    input.addEventListener('blur',()=>{if(select.value==='__custom')applyCustom()});
    form.addEventListener('submit',event=>{if(select.value==='__custom'&&!applyCustom()){event.preventDefault();input.focus()}},true);
  }

  function enhanceAllReminderForms(root=document){root.querySelectorAll?.('.profile-doc-form').forEach(enhanceReminderForm)}
  ...
  const observer=new MutationObserver(mutations=>{for(const mutation of mutations){for(const node of mutation.addedNodes){if(node.nodeType===1)enhanceAllReminderForms(node)}}});

  function init(){ensureStyles();ensurePhotoOverlay();enhanceAllReminderForms();observer.observe(document.documentElement,{childList:true,subtree:true})}
