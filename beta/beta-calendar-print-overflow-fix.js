...
function fixRoot(root,k){if(!root||!/^[0-9]{4}-[0-9]{2}$/.test(k))return;const doc=root.ownerDocument||document;ensureStyle(doc);const list=expiries();for(const cell of root.querySelectorAll('.hcp-cell:not(.hcp-empty)')){const day=Number(cell.querySelector('.hcp-num')?.textContent||0);if(!day)continue;const date=`${k}-${String(day).padStart(2,'0')}`;const due=list.filter(x=>x.expiryDate===date);if(!due.length)continue;const lines=cell.querySelector('.hcp-lines');if(!lines)continue;const shown=new Set([...lines.querySelectorAll('.hcp-line.expiry b')].map(b=>norm(b.textContent)));
 ...
 const more=lines.querySelector('.hcp-more');if(more)more.remove();const n=lines.querySelectorAll('.hcp-line').length;cell.classList.toggle('hcp-dense',n>=3);cell.classList.toggle('hcp-very-dense',n>=5);if((Number.parseInt(cell.closest('.hcp-weekrow')?.style?.getPropertyValue('--band-lanes')||'0',10)||0)>0)cell.classList.remove('hcp-header-share');}}
...
function bind(){const frame=document.querySelector('#homebaseCalendarPrintOverlay .hcp-frame');if(frame&&frame.dataset.overflow2385!=='1'){frame.dataset.overflow2385='1';frame.addEventListener('load',schedule,{passive:true})}}
