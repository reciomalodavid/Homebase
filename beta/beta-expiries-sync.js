...
function writeLocal(items,deleted,{reload=false}={}){
 ...
 if(reload){
   ...
   if(sessionStorage.getItem(marker)!==signature){sessionStorage.setItem(marker,signature);setTimeout(()=>location.reload(),80)}
 }
 return true;
}
...
async function syncNow(){
 ...
 syncing=true;
 try{await transactMerge()}catch(error){console.warn('Beta expiries sync',error)}finally{syncing=false;if(queued){queued=false;setTimeout(syncNow,120)}}
}
...
function attachListener(){
 const doc=ref();if(!doc||unsubscribe)return false;
 ...
 },error=>console.warn('Beta expiries listener',error));
 return true;
}
function install(){
 ...
 const start=()=>{
   tries++;
   if(db()&&syncCode()){
     attachListener();syncNow();return;
   }
   if(tries<80)setTimeout(start,250);
 };
 start();
 window.addEventListener('pageshow',()=>{if(!unsubscribe){attachListener();syncNow()}});
 document.addEventListener('visibilitychange',()=>{if(!document.hidden){if(!unsubscribe)attachListener();syncNow()}});
}
document.readyState==='loading'?document.addEventListener('DOMContentLoaded',install,{once:true}):install();
window.HOMEBASE_BETA_EXPIRIES_SYNC={version:VERSION,sync:syncNow};
})();
