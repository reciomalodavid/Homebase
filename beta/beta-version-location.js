...
function place(){
  ...
  if(!badge){
    ...
    badge.id='betaVersionBadgeInline';
  ...
  if(sync){
    ...
    if(!slot){
      ...
      slot.className='beta-version-loaded-slot';
      slot.style.cssText='padding:0 16px 10px;display:flex;justify-content:flex-end';
      const details=sync.querySelector('.sync-details');
      if(details)details.prepend(slot);else sync.appendChild(slot);
    }
  ...
  if(hero){
    let right=hero.querySelector('.beta-version-slot');
    if(!right){right=document.createElement('div');right.className='beta-version-slot';right.style.cssText='display:flex;align-items:center;justify-content:flex-end;margin-left:10px';hero.appendChild(right)}
...
function install(){
  place();
  document.addEventListener('click',e=>{if(e.target.closest('.bottom-nav,#syncSummary'))setTimeout(place,40)},true);
  window.addEventListener('pageshow',()=>setTimeout(place,0));
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)setTimeout(place,0)});
}
