(()=>{
  ...
  function persist(){
    ...
    renderSelectedDay();
    window.dispatchEvent(new CustomEvent('homebase:expiries-updated',{detail:{source:'production-v1'}}));
  }
  ...
  function ensureStyles(){
    ...
    style.textContent=`
      ...
      .profile-doc-form{display:none;margin-top:10px;padding:13px;border-radius:14px;background:var(--surface-2,#f5f5f5);border:1px solid var(--line,#ddd)}.profile-doc-form[hidden]{display:none!important}.profile-doc-form.open:not([hidden]){display:block!important}.profile-doc-form label{display:block;margin:0 0 5px;font-size:12px;font-weight:800}.profile-doc-form input,.profile-doc-form select,.profile-doc-form textarea{width:100%;box-sizing:border-box}.profile-doc-form textarea{min-height:68px}
  ...
  function wrapRender(name,options){
    ...
    if(typeof original!=='function'||original.__homebaseExpiriesWrapped)return;
    function wrapped(...args){
      const result=original.apply(this,args);
      queueRefresh(options);
      return result;
    }
    wrapped.__homebaseExpiriesWrapped=true;
    wrapped.__homebaseExpiriesOriginal=original;
    window[name]=wrapped;
  }
  ...
  function init(){
    ...
    window.addEventListener('homebase:expiries-updated',event=>{
      if(event.detail?.source==='production-v1')return;
      items=load();
